    ...
    // capture any errors that occur when writing data to the file
      ...
      countLinesInFile(path.join(__dirname, 'JSON', `temp_${region}.json`), (err, num) => {
        if (err) reject(err);
          ...
          fs.createReadStream(path.join(__dirname, 'JSON', `temp_${region}.json`), 'utf-8');

          const readline = readLine.createInterface({
            input: fs.createReadStream(path.join(__dirname, 'JSON', `temp_${region}.json`), 'utf-8'),
          });
      ...
      if (listErr) throw new Error(listErr);
        ...
        if (listData.IsTruncated) {
          ...
          for (const file of ti) {
            ...
            // merge first level object JSON with different regions into one first level object JSON
            try {
              await mergeFiles(inputFileArr, outputFile);
